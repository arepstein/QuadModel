function dstate=full_state_quad_EOMv2(t,state,quad_vars)
dstate=zeros(12,1);
%state=[x y z rho pi gamma u v w p q r]
%dstate=[xdot ydot zdot rhodot pidot gammadot udot vdot wdot pdot qdot
%rdot]
...
force.spring=.01*force.ground*1; %mN - make small contribution to upward force
...
%Equations of motion
% R_BG=[cos(state(6))*cos(state(5)) -sin(state(6))*cos(state(4))+cos(state(6))*sin(state(5))*sin(state(4)) sin(state(6))*sin(state(4))+cos(state(6))*cos(state(4))*sin(state(5));...
%                 sin(state(6))*cos(state(5)) cos(state(6))*cos(state(4))+sin(state(6))*sin(state(5))*sin(state(4)) -cos(state(6))*sin(state(4))+sin(state(6))*cos(state(4))*sin(state(5));...
%                 -sin(state(5)) cos(state(5))*sin(state(4)) cos(state(5))*cos(state(4))];
R_BG=RotMat([state(4) state(5) state(6)]);
T=[1 sin(state(4))*tan(state(5)) cos(state(4))*tan(state(5));...
    0 cos(state(4)) -sin(state(4));...
    0 sin(state(4))/cos(state(5)) cos(state(4))/cos(state(5))];
J=[R_BG zeros(3,3);...
    zeros(3,3) T];
dstate(1:6,1)=J*[state(7) state(8) state(9) state(10) state(11) state(12)]';
dstate(7,1)=force.body.x/quad.mass.magnitude-state(9)*state(11)+state(8)*state(12)-world.g*sin(state(5));
dstate(8,1)=force.body.y/quad.mass.magnitude+state(9)*state(10)-state(7)*state(12)+world.g*sin(state(4))*cos(state(5));
dstate(9,1)=force.body.z/quad.mass.magnitude-state(8)*state(10)+state(7)*state(11)+world.g*cos(state(4))*cos(state(5));
dstate(10,1)=(torque.body.x-quad.inertia.matrix(3,3)*state(11)*state(12)...
          +quad.inertia.matrix(2,2)*state(11)*state(12))/quad.inertia.matrix(1,1);  
dstate(11,1)=(torque.body.y+(state(10)*state(12))*(quad.inertia.matrix(3,3)-quad.inertia.matrix(1,1)))/...
    quad.inertia.matrix(2,2);
dstate(12,1)=(torque.body.z+(state(10)*state(11))*(quad.inertia.matrix(1,1)-quad.inertia.matrix(2,2)))/...
    quad.inertia.matrix(3,3);
